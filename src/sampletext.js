...
function roundTo(decimalNumber, decimalPlaces) {
  const Ten = 10;
  return Math.round(decimalNumber * Math.pow(Ten, decimalPlaces)) / (Math.pow(Ten, decimalPlaces));
}
function roundThree(decimalNumber) {
  const Three = 3;
  return roundTo(decimalNumber, Three);
}
function roundOne(decimalNumber) {
  return roundTo(decimalNumber, 1);
}
...
function stepFromClass(className) {
  if (!className) {
    return 0;
  }
  ...
  const rightOfDash = theClass.split('-')[1];
  return Number(rightOfDash || 0);
}
...
<div className="step-0 heading">
  step 0 abc def ghi jkl mno pqrs tuv wxyz ABC DEF GHI JKL MNO PQRS TUV WXYZ !"§ $%& /() =?*
  '{'<>'} #|; ²³~ @`´ ©«» ¤¼× {'{}'} abc def ghi jkl mno pqrs tuv wxyz ABC DEF GHI JKL MNO
  PQRS TUV WXYZ !"§ $%& /() =?* '{'<>'} #|; ²³~ @`´ ©«» ¤¼× {'{}'} abc def ghi jkl mno pqrs tuv
  wxyz ABC DEF GHI JKL MNO PQRS TUV WXYZ !"§ $%& /() =?* '{'<>'} #|; ²³~ @`´ ©«» ¤¼× {'{}'}abc
